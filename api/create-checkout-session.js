...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch profile to see if we already have a Stripe customer
    const { data: profile, error: pErr } = await supabase
      .from('profiles').select('email,stripe_customer_id').eq('user_id', user_id).maybeSingle();
    if (pErr) throw pErr;
    ...
    if (!customerId) {
      ...
      await supabase.from('profiles').update({ stripe_customer_id: customerId }).eq('user_id', user_id);
    }

    // Use environment price IDs you create in Stripe
    const priceId = plan === 'family'
      ? process.env.STRIPE_PRICE_FAMILY   // e.g. price_123
      : process.env.STRIPE_PRICE_INDIVIDUAL;

    if (!priceId) return res.status(500).json({ error: 'missing_price_id' });

    // 5-day trial via subscription_data.trial_period_days
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: {
        ...
        metadata: {
          plan,
          sb_user_id: user_id,
          seats: plan === 'family' ? 4 : 1
        }
      },
      metadata: { plan, sb_user_id: user_id },
